...
/**
 * A simple generator that combines an additive counter (Weyl sequence) with a 64-bit Xorshift generator for its two
 * state transitions, and outputs a value after using an ARX-only mixer.
 * <br>
 * This passes 128TB of PractRand with no anomalies. It does not use multiplication for its {@link #nextLong()} or other
 * core operations. It has a period of (2 to the 128) minus (2 to the 64), and is 1D-equidistributed.
 */
public class FlamencoRandom extends EnhancedRandom {
	...
	/**
	 * Creates a new FlamencoRandom with a random state.
	 */
	public FlamencoRandom() {
		super();
	...
	/**
	 * Creates a new FlamencoRandom with the given seed; all {@code long} values are permitted.
	 * The seed will be passed to {@link #setSeed(long)} to attempt to adequately distribute the seed randomly.
	 *
	 * @param seed any {@code long} value
	 */
	public FlamencoRandom(long seed) {
		super(seed);
		setSeed(seed);
	}

	/**
	 * Creates a new FlamencoRandom with the given two states; all {@code long} values are permitted except 0 for
	 ...
	 */
	public FlamencoRandom(long stateA, long stateB) {
		super(stateA);
		this.stateA = stateA;
		this.stateB = stateB;
		if (stateA == 0L)
	 ...
	 * Selection 0 refers to stateA; if the selection is anything
	 * else, this treats it as 1 and sets stateB. If stateA would be set to be 0, this
	...
	public long previousLong() {
		...
		stateA ^= stateA << 56;
		long y = stateA;
		long x = (stateB -= 0xC13FA9A902A6328FL) + y;
		x ^= (x << 19 | x >>> 35) ^ (x << 41 | x >>> 23);
		x += (y << 53 | y >>> 11);
		x ^= (x << 25 | x >>> 39) ^ (x << 50 | x >>> 14);
		x = (x << 31 | x >>> 33) + y;
	 ...
	 * <br>
	 * This returns the same result as {@link #nextLong()}, but changes {@link #getStateB() stateB} differently after
	...
	@Override
	public FlamencoRandom copy() {
		return new FlamencoRandom(stateA, stateB);
	}
	...
	public boolean equals(Object o) {
		...
		FlamencoRandom that = (FlamencoRandom) o;
	...
	public String toString() {
		return "FlamencoRandom{" + "stateA=" + (stateA) + "L, stateB=" + (stateB) + "L}";
	}

	public static void main(String[] args) {
		FlamencoRandom random = new FlamencoRandom(1L);
		{
