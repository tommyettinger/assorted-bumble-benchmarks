...
public class WowRandom extends EnhancedRandom {
	...
	/**
	 * Creates a new WowRandom with a random state.
	 */
	public WowRandom() {
	...
	/**
	 * Creates a new WowRandom with the given state; all {@code long} values are permitted.
	 *
	 * @param state any {@code long} value
	 */
	public WowRandom(long state) {
		super(state);
		this.state = state;
	}
	...
	public long nextLong () {
		...
		final long n = (x ^ (y << 39 | y >>> 25)) * (y ^ (x << 39 | x >>> 25));
		return n ^ n >>> 32;
	}
	...
	public int next (int bits) {
		...
		final long y = x ^ 0xE7037ED1A0B428DBL;
		final long n = (x ^ (y << 39 | y >>> 25)) * (y ^ (x << 39 | x >>> 25));
		return (int)(n ^ n >>> 32) >>> (32 - bits);
	}

	@Override
	public WowRandom copy () {
		return new WowRandom(state);
	}
	...
	public String toString () {
		return "WowRandom{state=" + (state) + "L}";
	}
}
