...
public class CongaRandom extends EnhancedRandom {
	...
	/**
	 * Creates a new CongaRandom with a random state.
	 */
	public CongaRandom() {
		super();
	...
	/**
	 * Creates a new CongaRandom with the given seed; all {@code long} values are permitted.
	 * The seed will be passed to {@link #setSeed(long)} to attempt to adequately distribute the seed randomly.
	 *
	 * @param seed any {@code long} value
	 */
	public CongaRandom(long seed) {
		super(seed);
		setSeed(seed);
	}

	/**
	 * Creates a new CongaRandom with the given two states; all {@code long} values are permitted for
	 * stateA and for stateB. These states are not changed during assignment.
	 *
	 * @param stateA any {@code long} value
	 * @param stateB any {@code long} value
	 */
	public CongaRandom(long stateA, long stateB) {
		super(stateA);
		this.stateA = stateA;
		this.stateB = stateB;
	}
	...
	public long nextLong () {
		long x = stateA;
		stateA = x * 0xC6BC279692B5CC83L ^ 0x9E3779B97F4A7C15L;
		x = (x << 20 | x >>> 44) + (stateB = stateB * 0xD1342543DE82EF95L + BitConversion.countLeadingZeros(x));
	...
	public int next (int bits) {
		...
		stateA = x * 0xC6BC279692B5CC83L ^ 0x9E3779B97F4A7C15L;
		x = (x << 20 | x >>> 44) + (stateB = stateB * 0xD1342543DE82EF95L + BitConversion.countLeadingZeros(x));
		return (int)(x ^ (x << 29 | x >>> 35) ^ (x << 47 | x >>> 17)) >>> (32 - bits);
	}

	@Override
	public CongaRandom copy () {
		return new CongaRandom(stateA, stateB);
	}
	...
	public boolean equals (Object o) {
		...
		CongaRandom that = (CongaRandom)o;

		return stateA == that.stateA && stateB == that.stateB;
	}

	public String toString () {
		return "CongaRandom{" + "stateA=" + (stateA) + "L, stateB=" + (stateB) + "L}";
	}

//	public static void main(String[] args) {
//		CongaRandom random = new CongaRandom(1L);
