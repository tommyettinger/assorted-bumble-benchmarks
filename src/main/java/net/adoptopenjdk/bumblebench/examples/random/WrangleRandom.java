...
/**
 * A hash-on-counters type of RNG with two 64-bit states; does not use multiplication, only add, XOR, and left-rotate
 * operations. This is like LaserRandom in many ways, but has much less correlation between starting states. Has an
 * exact period of 2 to the 64.
 */
public class WrangleRandom extends EnhancedRandom {
	@Override
	...
	/**
	 * Creates a new WrangleRandom with a random state.
	 */
	public WrangleRandom() {
		stateA = EnhancedRandom.seedFromMath();
		stateB = EnhancedRandom.seedFromMath();
	}

	/**
	 * Creates a new WrangleRandom with the given seed; all {@code long} values are permitted.
	 * The seed will be passed to {@link #setSeed(long)} to attempt to adequately distribute the seed randomly.
	 *
	 * @param seed any {@code long} value
	 */
	public WrangleRandom(long seed) {
		setSeed(seed);
	}

	/**
	 * Creates a new WrangleRandom with the given two states; all {@code long} values are permitted.
	 * These states will be used verbatim.
	 *
	 * @param stateA any {@code long} value
	 * @param stateB any {@code long} value
	 */
	public WrangleRandom(long stateA, long stateB) {
		this.stateA = stateA;
		this.stateB = stateB;
	}
...
//		y += (x ^ (x << 46 | x >>> 18) ^ (x << 21 | x >>> 43));
	...
	@Override
	public WrangleRandom copy () {
		return new WrangleRandom(stateA, stateB);
	}
	...
	public boolean equals (Object o) {
		...
		WrangleRandom that = (WrangleRandom)o;

		return stateA == that.stateA && stateB == that.stateB;
	}

	public String toString () {
		return "WrangleRandom{" + "stateA=" + (stateA) + "L, stateB=" + (stateB) + "L}";
	}

	public static void main(String[] args) {
		WrangleRandom random = new WrangleRandom(1L);
